...
namespace DotCast.Library.Specifications
{
    internal record AudioBookStatisticSpecification : ISpecification<AudioBook, AudioBooksStatistics>
    {
        ...
        {
            var data = await queryable.Select(t => new { Author = t.AudioBookInfo.AuthorName, t.AudioBookInfo.Duration.TotalHours }).ToListAsync(cancellationToken);
            var count = data.Count();
            var totalHours = data.Sum(x => x.TotalHours);
